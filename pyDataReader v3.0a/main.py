...
if __name__ == "__main__":
    ...
    while True:
        ...
        if re.match(r'^(\d{3} \d{6}[ ?]\d{6})$', magnetometer_reading):

            # get the current POSIX time
            reading_time = math.floor(time.time())

            lg = re.split('\?', magnetometer_reading)
            datavalue = float(lg[1]) / 10

            # create the datapoint. Print the values for the user.
            data_point = datapoint.DataPoint(reading_time, datavalue)
