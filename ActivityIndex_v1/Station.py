...
class Station:
    ...
    def normaliseDHDT(self):
        ...
        # for item in self.bin_dhdt:
        for i in range (0, len(self.bin_dadt)):
            if self.bin_dadt[i] == 0:
                temparray.append(0)
            elif self.bin_dadt[i] == NULLBIN:
                temparray.append(0)
            else:
                tempdata = (self.bin_dadt[i] - minvalue) / (maxvalue - minvalue)
