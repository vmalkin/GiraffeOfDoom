...
class PictureToProcess:
    ...
    def _return_pixelvalue(self, img):
        ...
        # bl = str(img[int(img_height / 2),int(img_width / 2), 0])
        # gr = str(img[int(img_height / 2), int(img_width / 2), 1])
        # rd = str(img[int(img_height / 2), int(img_width / 2), 2])
        # # array in format [blue, green, red]. closer to zero, closer to black
        # returnstring = rd + "," + gr + "," + bl
        # return returnstring

        pxl_array = str(img[int(img_height / 2),int(img_width / 2)])
        return pxl_array
